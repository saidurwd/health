<?php
$this->pageTitle = 'Medicine Bill Report';
$this->breadcrumbs = array(
    'Reports' => array('sales'),
    'Medicine Bill Report',
);
                ...
                <div>
                    ...
                    <div class="widget-body">
                        ...
                        <div class="row">
                            ...
                            <section class="col col-1">
                                <?php echo CHtml::link('<i class="fa fa-print"></i> Print', array('medicineprint', 'start_date' => $start_date, 'end_date' => $end_date, 'category' => $category, 'category_new' => $category_new, 'service' => $service, 'status' => $status), array('class' => 'btn btn-info btn-sm btn-block', 'target' => '_blank')); ?>
                            </section>
                        </div>
                        <?php $this->endWidget(); ?>
                        <table class="table table-bordered table-striped table-hover">
                            ...
                            <tbody>
                                <?php
                                $array = Report::medicineBillReport($start_date, $end_date, $category, $category_new, $service, $status);
                                $total = count($array);
                                $total_srv = 0;
                                $total_med = 0;
                                $total_amount = 0;
                                foreach ($array as $key => $value) {
                                    ...
                                    echo '<td class="text-right">' . Product::number_format_currency($value["amount_medicine"], 2, Yii::app()->session->get("currency")) . '</td>';
                                    echo '<td class="text-right">' . Product::number_format_currency(($value["amount_service"] + $value["amount_medicine"]), 2, Yii::app()->session->get("currency")) . '</td>';
                                    echo '</tr>';
                                    $total_srv += $value["amount_service"];
                                    $total_med += $value["amount_medicine"];
                                    $total_amount += ($value["amount_service"] + $value["amount_medicine"]);
                                }
                                ?>
                            </tbody>
                            <tfoot>
