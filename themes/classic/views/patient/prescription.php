...
<div style="margin-top: 20px;">
    ...
    <div style="width:425px;float:left;margin-left: 10px;">
        ...
        <?php
        $array = PrescriptionMedicine::model()->findAll(array('condition' => 'parent=' . (int) $_REQUEST['preid'], 'order' => 'created_on DESC'));
        $i = 1;
        echo '<h5 style="text-align:center;">PHARMACY ORDER</h5>';
        echo '<table class="table table-bordered table-striped table-hover">';
        echo '<thead>';
        echo '<tr><th>SL#</th><th>Medicine</th><th>Instruction</th><th>No of Days</th></tr>';
        echo '</thead>';
        echo '<tbody>';
        foreach ($array as $key => $values) {
            echo '<tr><td>' . $i . '</td>';
            echo '<td>' . $values["product"] . '</td>';
            echo '<td>' . $values["instruction"] . '</td>';
            echo '<td style="text-align:center;">' . $values["no_of_days"] . '</td>';
            $i++;
        }
        echo '</tbody>';
        echo '</table>';
        ...
        <p>
            <?php
            $array = InvoiceParent::model()->findAll(array('condition' => 'prescription=' . (int) $_REQUEST['preid'], 'order' => 'created_on DESC'));
            foreach ($array as $key => $values) {
                echo $values["invoice_number"];
                $array2 = Invoice::model()->findAll(array('condition' => 'parent=' . (int) $values["id"] . ' AND servicetype="Medicine"'));
                echo '<ul>';
                foreach ($array2 as $key2 => $values2) {
                    echo '<li>' . Product::getData($values2["item"], 'title') . ' ' . round($values2["quantity"], 0) . Product::getItemUOM($values2["item"]) . '</li>';
                }
                echo '</ul>';
                $array3 = Invoice::model()->findAll(array('condition' => 'parent=' . (int) $values["id"] . ' AND servicetype="Service"'));
                echo '<ul>';
                foreach ($array3 as $key3 => $values3) {
                    echo '<li>' . Service::getData($values3["service"], 'title') . ' ' . round($values3["quantity"], 0) . 'unit</li>';
                }
                echo '</ul>';
            }
            ?>
