 ...
 *
 * The followings are the available columns in table '{{patient_category}}':
 * @property integer $id
 * @property string $title
 * @property string $status
 */
class PatientCategory extends CActiveRecord {
     ...
     * @return PatientCategory the static model class
     */
    ...
    public static function getData($id, $field) {
        $model = PatientCategory::model()->findByAttributes(array('id' => $id));
    ...
    public static function getPatientCategory($model, $field, $id) {
        $parent1 = PatientCategory::model()->findAll(array('condition' => 'parent=0 OR parent IS NULL', 'order' => "title"));
        $option = '<select id="' . $model . '_' . $field . '" name="' . $model . '[' . $field . ']" class="select2">';
        $option .= '<option value="">Select a Sub Category</option>';
        foreach ($parent1 as $key => $values1) {
            $option .= '<optgroup label="' . $values1["title"] . '">';
            $parent2 = PatientCategory::model()->findAll(array('condition' => 'parent=' . (int) $values1["id"], 'order' => 'title'));
    ...
    public static function update_path($id) {
        $model = PatientCategory::model()->findByPk($id);
        if ($model->parent == 0 || $model->parent === null) {
            $model->path = '0.' . $model->id;
            $model->save();
        } else {
            $parent = PatientCategory::model()->findByAttributes(array('id' => $model->parent));
    ...
    public static function get_full_path($id) {
        ...
        foreach ($array as $key => $value) {
            if ($value > 0) {
                if ($total != $i) {
                    $data .= PatientCategory::getData($value, 'title') . ' <i class="fa fa-angle-double-right text-info"></i> ';
                } else {
                    $data .= PatientCategory::getData($value, 'title');
                }
            }
            $i++;
        }
