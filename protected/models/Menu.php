...
class Menu extends CActiveRecord {
    ...
    public static function get_menu() {
        ...
        foreach ($parent1 as $key => $values1) {
            if ($values1["parent"] == 0) {
                echo '<li class="' . Menu::get_active_menu($values1["url"]) . '">';
            } else {
    ...
    public static function get_menu_new() {
        $parent1 = Yii::app()->db->createCommand()
                ...
                ->order('parent,title')
                ->queryAll();
        $option = '<select id="Menu_parent" name="Menu[parent]" class="form-control">';
...
//get menu for view
    ...
    public static function get_icon($id) {
        $value = Menu::model()->findByAttributes(array('id' => $id));
        if (empty($value->icon)) {
            return null;
        } else {
