...
class District extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            array('state, city, title', 'required'),
            array('country, state, city', 'numerical', 'integerOnly' => true),
            array('title', 'length', 'max' => 150),
            array('status', 'length', 'max' => 8),
            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            array('id, country, state, city, title, status', 'safe', 'on' => 'search'),
        );
    }
    ...
    public static function getData($id, $field) {
        $model = District::model()->findByAttributes(array('id' => $id));
    ...
    public static function getRelatedDistrictCountry($model, $field, $id, $class = 'form-control') {
        ...
        foreach ($array as $key => $values) {
            if ($values["id"] == $id) {
                $option .= '<option selected="selected" value="' . $values["id"] . '" class="' . $values["country"] . '">' . $values["title"] . '</option>';
            } else {
                $option .= '<option value="' . $values["id"] . '" class="' . $values["country"] . '">' . $values["title"] . '</option>';
            }
        }
        $option .= '</select>';
