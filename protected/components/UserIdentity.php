...
class UserIdentity extends CUserIdentity {
    ...
    public function authenticate() {
        ...
        if ($user === null) { // No user found!
            ...
            Yii::app()->db->createCommand('UPDATE {{user}} SET `lastvisit` = NOW() WHERE id=' . $user->id)->execute();
            $this->errorCode = self::ERROR_NONE;
            // Store the role in a session:
            $this->setState('group', $user->group_id);
            $this->setState('email', $user->email);
            $this->setState('fullname', $user->full_name);
            Yii::app()->session->add('currency', '৳');
            $this->_id = $user->id;
            Yii::app()->db->createCommand('UPDATE {{yiisession}} SET `userId` = ' . $user->id . ' WHERE id="' . session_id() . '"')->execute();
        }
